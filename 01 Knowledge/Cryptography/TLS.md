...
При разборе соединения полезно отдельно проверять доступность транспорта, успешность Handshake, проверку сертификата и передачу защищённых Record. Успешное сетевое соединение само по себе ещё не подтверждает подлинность второй стороны.
