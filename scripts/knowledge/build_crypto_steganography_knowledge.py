...
add(
    ...
    example="Лекция показывает, что глаз чувствительнее к яркостной составляющей; JPEG использует это при прореживании цветности перед блочной ДКП.",
...
add(
    ...
    example="В лекции один и тот же блок пикселей преобразуется с помощью ДПФ, ДКП и преобразования Уолша—Адамара, после чего сравниваются спектры и результат восстановления.",
...
add(
    ...
    example="Курс рассматривает изображения как основной контейнер и отделяет сокрытие сообщения от встраивания водяного знака для подтверждения происхождения или целостности.",
...
add(
    ...
    example="Из матрицы 2×2 курс строит интерполированную матрицу 4×4 и встраивает `0001101111001011`; первый фрагмент меняет значение 119 на 122.",
...
add(
    ...
    example="Курс последовательно рассматривает JSteg, PM1, F3, F4 и F5 как способы уменьшить статистические следы и корректно обработать обнуление квантованных коэффициентов.",
...
add(
    ...
    summary="F5 JPEG Steganography применяет матричное кодирование к группам коэффициентов: несколько битов можно встроить, изменив не более одного коэффициента группы. Поэтому на каждый бит сообщения приходится меньше изменений.",
...
add(
    ...
    example="Лекция проходит путь от просмотра LSB-плоскостей до CNN и подчёркивает необходимость выборки, соответствующей конкретным контейнерам и методам встраивания.",
...
add(
    ...
    example="После LSB-встраивания младшая плоскость на слайдах теряет естественную структуру; JSteg выравнивает отдельные столбцы гистограммы коэффициентов ДКП.",
...
add(
    ...
    example="Курс сравнивает результаты теста пар значений для чистого и наполовину заполненного изображения, а затем перечисляет 23 признака JPEG классического подхода.",
...
add(
    ...
    example="Курс описывает 23 признака JPEG, затем сравнивает k-NN и наивный байесовский классификатор; закон Бенфорда приведён как дополнительный статистический признак.",
